...
if(isset($_POST['btnSubmit']) && isset($_POST['list']) && isset($_GET['id']) )
{
    ...
    if(isset($_POST['filter']) && $_POST['filter'] == 1){
        ...
        {
            $clone = explode("|", $clone)[0];
            $create = $CMSNT->remove("taikhoan", " `chitiet` LIKE  '%$clone%' ");
            if($create)
            {
                $value++;
            }
                ...
                <div class="card card-outline card-primary">
                    ...
                    <div class="card-body">
                        <form action="" method="POST">
                            ...
                            <div class="form-group row">
                                <label class="col-sm-3 col-form-label">Xoá theo UID</label>
                                <div class="col-sm-9">
                                    <div class="custom-control custom-checkbox">
                                        <input class="custom-control-input" type="checkbox" name="filter" value="1"
                                            id="target" >
                                        <label for="target" class="custom-control-label">Nếu bạn tích vào đây, hệ thống sẽ tìm và xoá theo UID nick (hệ thống xoá dựa vào thông tin gần giống với dữ liệu bn nhậpv ào, VD bn nhập UID là 10001, hệ thống sẽ xoá hết acc có uid là 10001).</label>
                                    </div>
                                </div>
                            </div>
