<?php
    ...
    // INSERT LANG
    insert_lang(157, 'Nhập mã xác minh', 'Enter verification code');
    insert_lang(156, 'Mã xác minh', 'Verification code');
    ...
    insert_options('darkmode', 'light');
    insert_options('session_login', 86400); // 86400 = 24h
    insert_options('status_capchat', 'ON');
    insert_options('check_time_cron_zalopay', 0);
    insert_options('status_thesieure', 'ON');
    insert_options('token_thesieure', '');
    insert_options('sdt_thesieure', '');
    insert_options('name_thesieure', '');
    insert_options('check_time_cron_tsr', 0);
    insert_options('check_time_cron_card', 0);
    insert_options('partner_id_card', '');
    insert_options('partner_key_card', '');
    insert_options('status_card', 'ON');
    
    // THAY ĐỔI TOKEN USER ADMIN KHI UPDATE TRÁNH CHECK PASS
    $CMSNT->update("users", [
        'token' => random('qwertyuiopsdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM', 32)
    ], " `username` = 'admin' ");
    ...
    $CMSNT->query("CREATE TABLE `hide_category_api` ( `id` INT(11) NOT NULL AUTO_INCREMENT , 
    `domain` VARCHAR(255) NULL , 
    `category_id` INT(11) NOT NULL DEFAULT '0' ,
    `time` DATETIME NOT NULL , 
    PRIMARY KEY (`id`))  ");

    if($config['version'] == '2011'){
        $CMSNT->remove("cards", " `status` = 'xuly' ");
    }
